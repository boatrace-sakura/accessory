    ...
    public function times(int $raceNumber, ?string $date = null): array
    {
        ...
        foreach (range(1, 6) as $bracket) {
            $response['bracket' . $bracket . 'RacerName'] =
                preg_split('/\d{4}/u', $this->removeSpace($times['td.name'][$bracket - 1] ?? ''))[0];
            $response['bracket' . $bracket . 'ExhibitionTime'] =
                (float) ($times['.choku_top'][$bracket * 2 - 2] ?? 0);
            $response['bracket' . $bracket . 'LapTime'] =
                (float) ($chunkTimes[$bracket - 1][0] ?? 0);
            $response['bracket' . $bracket . 'TurnTime'] =
                (float) ($chunkTimes[$bracket - 1][1] ?? 0);
            $response['bracket' . $bracket . 'StraightTime'] =
                (float) ($chunkTimes[$bracket - 1][2] ?? 0);
            $response['bracket' . $bracket . 'LapTimeSpeed'] =
                (float) ($chunkTimes[$bracket - 1][3] ?? 0);
            $response['bracket' . $bracket . 'TurnTimeSpeed'] =
                (float) ($chunkTimes[$bracket - 1][4] ?? 0);
            $response['bracket' . $bracket . 'StraightTimeSpeed'] =
                (float) ($chunkTimes[$bracket - 1][5] ?? 0);
        }
