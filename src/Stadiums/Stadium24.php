...
class Stadium24 extends BaseStadium implements StadiumInterface
{
    ...
    public function times(int $raceNumber, ?string $date = null): array
    {
        ...
        foreach (range(1, 6) as $bracket) {
            $response['bracket' . $bracket . 'LapTime'] = $times['.tei' . $bracket][4];
            $response['bracket' . $bracket . 'TurnTime'] = $times['.tei' . $bracket][5];
            $response['bracket' . $bracket . 'StraightTime'] = $times['.tei' . $bracket][6];
        }
